...
v.params <- c("P2","P3","P6","P6R","P7","P7R","PBMT")
v.ss <- c(50,75,100,250,1000)
v.cens <- c(0.999,0.50,0.10,0.010)
v.sims <- c(10000)
v.seed <- c(2023)
...
print(ThisJob)
### Assign paramater lists and correlation matrices

if (ThisJob$Var1 == "P2") {
  params <- list(1,1,1,1,1e-2,c(0,qexp(ThisJob$Var3, rate=1e-2)))
} else if (ThisJob$Var1 == "P3") {
  params <- list(1,1e-1,1,1e-1,1e-2,c(0,qexp(ThisJob$Var3, rate=1e-2)))
} else if (ThisJob$Var1 == "P6") 
  {params <- list(1,1,1e-1,1,1e-2,c(0,qexp(ThisJob$Var3, rate=1e-2)))
} else if (ThisJob$Var1 == "P6R") {
  params <- list(1e-1,1,1,1,1e-2,c(0,qexp(ThisJob$Var3, rate=1e-2)))
} else if (ThisJob$Var1 == "P7") {
  params <- list(1,1,1,1e-1,1e-2,c(0,qexp(ThisJob$Var3, rate=1e-2)))
} else if (ThisJob$Var1 == "P7R") {
  params <- list(1,1e-1,1,1,1e-2,c(0,qexp(ThisJob$Var3, rate=1e-2)))
} else if (ThisJob$Var1 == "PBMT") {
  params <- list(1/150,1/10,1/150,1/10,1/(365.25*3),c(0,qexp(ThisJob$Var3, rate=1/(365.25*3))))
}
